...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        shit = [x.eval(lookup) for x in self._subtrees]
    ...
    def __str__(self) -> str:
        ...
        if isinstance(self._root, str) and self._root.isalpha():
            return self._root
        shit = list(self._subtrees)
        a = ''
        if shit:
            a = str(shit[0])
            for i in range(1, len(shit)):
                a += ' ' + self._root + ' ' + str(shit[i])
        return '(' + a + ')'
    ...
    def __eq__(self, other: ExprTree) -> bool:
        ...
        if not self.is_empty() and other.is_empty():
            ...
            if self._root != other._root:
                return False
            if self._subtrees != other._subtrees:
                return False
        return True
