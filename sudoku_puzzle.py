...
class SudokuPuzzle(Puzzle):
    ...
    def _helper(self, curr_c: int, curr_r: int, edge: int) -> List[int]:
    ...
    def fail_fast(self) -> bool:
        ...
        curr_c = 0
        while curr_r != n:
            # while we did not exceed the range of this sudoku
            # check every position
            curr_pos = symbols[curr_r][curr_c]
            if curr_pos != EMPTY_CELL:
                # Handle with non-empty cases: pass to the next position
                r_c_alteration = self._helper(curr_c, curr_r, edge)
                curr_r = r_c_alteration[0]
                curr_c = r_c_alteration[1]

            else:
                # Handle with empty cases: get possible solutions for this
                # position, and pass to the next position
                allowed_symbols = (self._symbol_set
                                   - (self._row_set(curr_r)
                                      | self._column_set(curr_c)
                                      | self._subsquare_set(curr_r,
                                                            curr_c)))
    ...
    def has_unique_solution(self) -> bool:
        ...
        if not self.fail_fast():
            first_try = DfsSolver().solve(self)
            # solve for the first time， check if it can be solved
            if first_try:  # this sudoku can have at least one solution
                first_sol_end = first_try[-1]
                second_try = DfsSolver().solve(self,
                                               {str(first_sol_end)})
                if not second_try:
                    return True
        return False
