...
class WordLadderPuzzle(Puzzle):
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        for i in range(len(self.from_word)):
            for char in self._chars:
                create = self.from_word[:i] + char + self.from_word[i + 1:]
                if create in self.word_set and create != self.from_word:
                    move = WordLadderPuzzle(create, self.to_word, self.word_set)
                    moves.append(move)
        moves = sorted(moves,
                       key=lambda x: len(set(x.from_word) - set(x.to_word)))
        return moves
    ...
    def get_difficulty(self) -> str:
        ...
        """
        solution = BfsSolver().solve(self)
        if solution:
            if len(solution) <= 2:
                return TRIVIAL
            elif len(solution) == 3:
                return EASY
            elif 6 > len(solution) > 3:
                return MEDIUM
            elif len(solution) >= 6:
                return HARD
        return IMPOSSIBLE
