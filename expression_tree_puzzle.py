...
class ExpressionTreePuzzle(Puzzle):
    ...
    def is_solved(self) -> bool:
        ...
        for i in self.variables:
            if self.variables[i] <= 0 or self.variables[i] >= 10:
                return False
    ...
    def __str__(self) -> str:
        ...
        """
        return '{variable}\n{equa} = {target}'.\
            format(variable=self.variables, equa=self._tree.__str__(),
                   target=self.target)
    ...
    def extensions(self) -> List[ExpressionTreePuzzle]:
        ...
        """
        result = []
        expression_tree = self._tree.copy()
        variable = self.variables.copy()
        for i in variable:
            if variable[i] == 0:
                for j in range(1, 10):
                    variable[i] = j
                    puz = ExpressionTreePuzzle(expression_tree, self.target)
                    puz.variables = variable.copy()
                    result.append(puz)
                variable[i] = 0
        return result
    ...
    def fail_fast(self) -> bool:
        ...
        """
        if self.variables == {} \
                and self._tree.eval(self.variables) == self.target:
            return False
        if self.variables == {} \
                and self._tree.eval(self.variables) != self.target:
            return True
        if self.variables != {}:
            variable_copy_min = self.variables.copy()
            variable_copy_max = self.variables.copy()
            for i in variable_copy_min:
                if variable_copy_min[i] == 0:
                    variable_copy_min[i] = 1
            minvalue = self._tree.eval(variable_copy_min)
            for i in variable_copy_max:
                if variable_copy_max[i] == 0:
                    variable_copy_max[i] = 9
            maxvalue = self._tree.eval(variable_copy_max)
            if self.variables == variable_copy_min \
                    and self._tree.eval(self.variables) != self.target:
                return True
            if self.target <= minvalue or self.target >= maxvalue:
                return True
        return False
